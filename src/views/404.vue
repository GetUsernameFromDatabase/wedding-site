...
<template>
  <div
    ...
    @mousemove="
      (event) => {
        if (!torch) return;
        torch.style.top = event.pageY.toString() + 'px';
        torch.style.left = event.pageX.toString() + 'px';
      }
    "
  >
    <div class="text">
      <h1>404</h1>
      <h2>{{ t('c404.ohno') }}</h2>
      <h3>{{ t('c404.notfound') }}</h3>
    </div>
    <div ref="torch" class="torch"></div>
  </div>
</template>
...
.text h1 {
  ...
  font-size: 15em;
  text-align: center;
  text-shadow: -5px 5px 0px rgba(0, 0, 0, 0.7), -10px 10px 0px rgba(0, 0, 0, 0.4),
